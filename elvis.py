...
class Elvis(object):
    ...
    def __init__(self):
        ...
        # Команды установки регистра и записи
        self.COMMAND_SETA = 0x0010
        self.COMMAND_WR = 0x0020
        # Выбор профиля канала 1
        self.SEL_REG = 0x0002
    ...
    def switch_CH1_state(self, port_index):
        """Функция смены статуса 1 канала на новый.
        Статус канала может быть либо ВКЛ, либо ВКЛ, но в связи с тем,
    ...
    def switch_CH2_state(self, port_index):
        """Функция смены статуса 2 канала на новый.
        Статус канала может быть либо ВКЛ, либо ВКЛ, но в связи с тем,
        что на микросхеме не реализован вывод данных, то весь процесс
        сохраняется только в сессии программы.
